...
function getScriptExecutor() {
  if (typeof window.IntegrityGuardHost?.execScript === "function") {
    return (scriptPath, scriptName, cb) => {
      Promise.resolve(window.IntegrityGuardHost.execScript(scriptPath, scriptName))
        .then(result => cb(typeof result === "string" ? result : JSON.stringify(result)))
        .catch(() => cb(""));
    };
  }

  if (typeof window.execIntegrityGuardScript === "function") {
    return (scriptPath, scriptName, cb) => {
      Promise.resolve(window.execIntegrityGuardScript(scriptPath, scriptName))
        .then(result => cb(typeof result === "string" ? result : JSON.stringify(result)))
        .catch(() => cb(""));
    };
  }
